#!/usr/bin/env bash
mvn -Dbrowser=FIREFOXMAC -Dreport="nogridbbcnews" -DnoImages -DaNumber=150 -DdefaultImplicitWaitSeconds=30 -DnoScreenShots -DnoScroll -Dinput="http://www.bbc.com/news" -DbodyTextXpath="//div[contains(@class,'body')]/p" -DlinkXpath="//a[contains(@href,'/news/') and contains(@class,'-link') and not(contains(@href,'video_and_audio'))]" -DnextLinkXpath="//void" -DlinksLoadedIndicatorXpath="//span[contains(text(),'Copyright')]" -DimageXpath="//img" -DtitleTextXpath="//h1" -DaHubServer="localhost" -DaHubPort=4444 -Dtest="BoardScrub#BuildPageOfFoundLinks" test
#=====================================
