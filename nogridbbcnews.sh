#!/usr/bin/env bash
mvn -Dbrowser=CHROME -Dreport="nogridbbcnews" -DnoImages -DaNumber=150 -DdefaultImplicitWaitSeconds=60 -DnoScreenShots -DnoScroll -Dinput="http://www.bbc.com/news" -DbodyTextXpath="//div[contains(@class,'body')]/p" -DlinkXpath="//a[contains(@href,'/news/') and contains(@class,'gs-c-promo-heading') and not(contains(@href,'video_and_audio'))]" -DnextLinkXpath="//void" -DlinksLoadedIndicatorXpath="//em[contains(text(),'Copyright')]" -DimageXpath="//img" -DtitleTextXpath="//h1" -Dnogrid -Dtest="BoardScrub#BuildPageOfFoundLinks" test
#=====================================
